...
static enum ms8607_status hsensor_humidity_conversion_and_read_adc( uint16_t *);	//MEASURE RH HOLD/NO HOLD
...
int main (void)
{
	...
	while(1)
	{
		...
		sprintf(dsp_buff1, "P:  %4d T: %4d", p, t);
		sprintf(dsp_buff2, "H: %4d", h);
		sprintf(dsp_buff3, "Time:           ");
		update_lcd_dog();
	}
}
...
enum ms8607_status hsensor_humidity_conversion_and_read_adc( uint16_t *adc)
{
	...
	if( hsensor_i2c_master_mode == ms8607_i2c_hold) {
		...
		_delay_ms(hsensor_conversion_time/1000);
	}
	if( status != ms8607_status_ok)
		return status;
...
static enum ms8607_status psensor_conversion_and_read_adc(uint8_t cmd, uint32_t *adc)
{
	...
	_delay_ms( psensor_conversion_time[ (cmd & PSENSOR_CONVERSION_OSR_MASK)/2 ]/1000 );
	if( status != ms8607_status_ok)
		return status;
...
{
	enum ms8607_status status = ms8607_status_ok;
	uint32_t adc_temperature, adc_pressure;	//D2, D1
	int32_t dT, TEMP;
